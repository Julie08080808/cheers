...
| 喝酒判定方式 | 遊戲結果直接喝 | 依規則分配 |
| 回合結算 | 無固定結算 | 觸發喝酒事件算一回合 |
| 結束條件 | 任一玩家喝到 3 杯 | 5回合後結束 |
| 積分最高者權限 | 不適用 | 游戲結束後顯示贏家輸家 |
| 積分最低者懲罰 | 輸了直接喝 | 結算後喝酒 |
...
在系統架構上，我們先完成網路連線與房間流程：玩家加入房間、等待人數到齊、由房主開始遊戲，並透過抽籤／轉盤決定出手順序與開局設定。接著實作核心遊戲邏輯，以兩顆六面骰作為事件觸發依據，依據不同點數組合對應不同遊戲事件，並支援兩種模式：酒鬼模式（無積分、輸了直接喝、喝到三杯結束）與闔家歡模式（積分制、每五回合結算一次）。

在硬體實作部分，樹莓派透過 GPIO 輸出控制訊號，連接至 L298N／Relay 模組以驅動多顆幫浦。每一顆幫浦對應到特定 GPIO 腳位與控制通道，系統會將遊戲結果轉換為「酒款（幫浦選擇）＋出酒時間（秒數）」等控制參數，並由控制模組執行倒酒動作。至此，我們完成了從玩家端互動、伺服器判定，到實體調酒輸出的完整閉環 ~！
